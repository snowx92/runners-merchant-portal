...
export const MessageDrawer = () => {
    ...
    chatUnsubscribeRef.current = onSnapshot(
      ...
      (snapshot) => {
        ...
        const chatsData: Message[] = snapshot.docs.map((doc) => {
          ...
          const usersInfo = data['usersInfo'] as Record<string, { avatar?: string; name?: string }> | undefined;
        ...
        chatsData.sort((a, b) => {
          ...
          } else if (a.timestamp && typeof a.timestamp === 'object' && 'seconds' in a.timestamp) {
            timeA = (a.timestamp as { seconds: number }).seconds * 1000;
          } else if (a.timestamp && typeof a.timestamp === 'object' && '_seconds' in a.timestamp) {
          ...
          } else if (b.timestamp && typeof b.timestamp === 'object' && 'seconds' in b.timestamp) {
            timeB = (b.timestamp as { seconds: number }).seconds * 1000;
          } else if (b.timestamp && typeof b.timestamp === 'object' && '_seconds' in b.timestamp) {
    ...
    messagesUnsubscribeRef.current = chatService.subscribeToMessages(
      ...
      (msgs: ChatMessage[]) => {
        const uiMessages: ChatMessageUI[] = msgs.map((msg) => ({
          ...
          type: msg.imageUrl ? 'image' : msg.audioUrl ? 'audio' : msg.location ? 'location' : 'text',
  ...
  const handleSendMessage = async () => {
    if (!messageInput.trim() || !selectedChat?.chatId || !currentUserIdRef.current) return;
  ...
  const sendVoiceMessage = async () => {
    if (!audioBlob || !selectedChat?.chatId || !currentUserIdRef.current) return;
  ...
  const sendLocation = async () => {
    if (!userLocation || !selectedChat?.chatId || !currentUserIdRef.current) return;
    ...
    try {
      await chatService.sendMessageWithMedia(
        ...
        undefined,
        undefined,
        { latitude: userLocation.lat, longitude: userLocation.lng, address: userLocation.address }
      );
                  ...
                  >
                    <div
                      className={`${styles.chatMessageBubble} ${
                        msg.isOwn
                          ? (msg.type === 'text' ? styles.ownMessageBubble : styles.ownMessageBubbleAttachment)
                          : (msg.type === 'text' ? styles.otherMessageBubble : styles.otherMessageBubbleAttachment)
                      }`}
                    >
                      ...
                      {msg.type === 'location' && msg.location && (
                        <div style={{ marginBottom: '8px', background: 'transparent' }}>
                          <a 
                      ...
                      {msg.type === 'audio' && msg.audioUrl && (
                        <div style={{ marginBottom: '8px', background: 'transparent' }}>
                          <audio controls src={msg.audioUrl} style={{ width: '200px', background: 'transparent' }} />
                        </div>
                      )}
            ...
            <div className={styles.chatInput}>
              ...
              {audioBlob && (
                <div className={styles.voicePreview}>
                  ...
                  <button className={styles.cancelRecordingButton} onClick={cancelRecording}>
                ...
                <>
                  ...
                  {showAttachmentMenu && (
                    <div className={styles.attachmentMenu}>
                      <label className={`${styles.attachmentMenuItem} ${isUploadingImage ? styles.sendingButton : ''}`} style={{ background: 'transparent' }}>
                        {isUploadingImage ? (
